...
{
    /* the trick: we turn off this IRQ but on read we check whether the IRQ
        ...
        loff_t *fpos)
{
    ...
    //printk(KERN_WARNING "driver: pos=%llu\n", filep->f_pos);
    ...
    uart->irq_pending=0;
    /* in FIFO mode uint32_t can be obtained */
    memset(uart->r_buff, 0, RBUFF_SZ);
    *uart->r_buff=ioread32(PL011_DR(uart->iomem));
    if( copy_to_user((uint32_t *)data, uart->r_buff, sizeof(uint32_t)) )
    {
        err = -EFAULT;
        goto out;
    }
